...
void reference_affine_matmul(float * C, const float * A, const float * B, const float * d,
...
int test_k2c_affine_matmul() {
    ...
    float* d = (float*)malloc(outcols * sizeof(float));
    ...
    for (size_t i = 0; i < outcols; ++i) {
        d[i] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f;  // Random values between -1 and 1
    }

    // Perform the affine matrix multiplication
    k2c_affine_matmul(C, A, B, d, outrows, outcols, innerdim);
    reference_affine_matmul(C_ref, A, B, d, outrows, outcols, innerdim);
...
int main() {
    ...
    int result = test_k2c_affine_matmul();
